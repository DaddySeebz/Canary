  ...
  return (
    <div className="space-y-4">
      {entries.map((entry) => {
        ...
        return (
          <div key={entry.id} className="flex gap-4 rounded-[0.75rem] border border-[color:var(--workspace-border)] bg-slate-50 p-4 sm:p-5">
            ...
            <div className="min-w-0 space-y-1">
              ...
              {entry.details ? <pre className="whitespace-pre-wrap break-words font-mono text-xs text-slate-500">{entry.details}</pre> : null}
            </div>
          </div>
        );
      })}
