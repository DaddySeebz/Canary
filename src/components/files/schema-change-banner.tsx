        ...
        <div className="min-w-0 space-y-3">
          ...
          <div className="grid gap-3 text-sm md:grid-cols-3">
            <div>
              ...
              <div className="break-words">{schemaDiff.added.join(", ") || "None"}</div>
            </div>
            <div>
              <div className="mb-1 text-xs uppercase tracking-[0.18em] text-amber-700/80">Removed</div>
              <div className="break-words">{schemaDiff.removed.join(", ") || "None"}</div>
            </div>
            <div>
              ...
              <div className="space-y-1">
                {schemaDiff.renamed.length > 0
                  ? schemaDiff.renamed.map((rename) => (
                      <div key={`${rename.from}-${rename.to}`} className="inline-flex flex-wrap items-center gap-1">
                        <span className="break-words">{rename.from}</span>
                        <ArrowRight className="h-3 w-3" />
                        <span className="break-words">{rename.to}</span>
                      </div>
                    ))
                  : "None"}
              </div>
